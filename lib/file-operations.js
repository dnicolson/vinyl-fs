...
function findSymlinkHardpath(path, callback) {
  ...
  function onReadlink(readlinkErr, resolvedPath) {
    ...
    // Resolve relative symlink target relative to the symlink's directory
    if (!require('path').isAbsolute(resolvedPath)) {
      resolvedPath = require('path').resolve(require('path').dirname(path), resolvedPath);
    }
...
function reflectLinkStat(path, file, callback) {
  ...
  if (!require('path').isAbsolute(path) && file.cwd) {
    absolutePath = require('path').resolve(file.cwd, path);
  }
  fs.lstat(absolutePath, onLstat);
